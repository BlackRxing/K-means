...
public class KMeans {
    ...
    private float disDiff = (float) 0;         //单次迭代终止条件，两次运行中类中心的距离差
    ...
    public void cluster(Set<Cluster> clusterSet){
        ...
        for (Point point : pointList) {
            ...
            for (Cluster cluster : clusterSet) {
                float tmp_dis = (float) Math.min(getEuclideanDis(point, cluster.getCenter()), min_dis);
                if (tmp_dis != min_dis) {
                    min_dis = tmp_dis;
    ...
    public boolean calculateCenter(Set<Cluster> clusterSet) {
        ...
        for (Cluster cluster : clusterSet) {
            ...
            if(getEuclideanDis(cluster.getCenter(), new Point(sumAll)) > disDiff){
                ifNeedIter = true;
            }
            // 设置新的类中心位置
